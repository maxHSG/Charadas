...
export default class RadioGroup extends Component {
	constructor(props) {
		super(props);
		this.state = { radioButtons: this.validate(this.props.radioButtons) };
	}

	componentWillReceiveProps(nextProps) {
		// You don't have to do this check first, but it can help prevent an unneeded render
		console.log("nextProps", nextProps);
		if (nextProps.radioButtons !== this.state.radioButtons) {
			this.setState({
				radioButtons: this.validate(nextProps.radioButtons)
			});
		}
	}
	validate(data) {
	...
	onPress = async label => {
		...
		if (selectedIndex != selectIndex) {
			...
			await this.setState({ radioButtons });
		...
		return (
			<TouchableOpacity
				style={[
					layout,
					data.styleRadio,
					{ opacity, marginHorizontal: 10, marginVertical: 5 }
				]}
				onPress={() => {
